...
describe("money formatter", () => {
  it("formats int64 values without converting the whole value to number", () => {
    expect(formatMinorAmount(MAX_INT64_MINOR)).toBe("R$ 92.233.720.368.547.758,07");
  });
...
describe("MoneyInput", () => {
  it("renders a visible label and a currency prefix", () => {
    const markup = renderToStaticMarkup(<MoneyInput id="amount" label="Valor" />);
  ...
  it("preserves disabled state on the input and group", () => {
    const markup = renderToStaticMarkup(<MoneyInput id="amount" label="Valor" disabled />);
