...
describe("investment command contracts", () => {
  it("carries a stable request envelope", () =>
    expect(base).toMatchObject({ bookId: "book-1", requestId: "request-1" }))
  it("models internal funding without an account", () => {
    const draft: PurchaseOrApplicationDraft = {
      ...base,
      type: "PURCHASE",
      capitalMinor: "1000",
  ...
  it("returns every negative-cash warning field", () => {
    const warning: InvestmentWarning = {
  ...
  it("preserves a replayable mutation result in a versioned receipt", () => {
    const result: InvestmentMutationResult = {
      requestId: "request-1",
      positionId: "position-1",
      journalEntryIds: ["journal-1"],
      warnings: [],
    }
    const receipt: InvestmentRequestReceipt = {
      bookId: "book-1",
      requestId: "request-1",
      formatVersion: 1,
      canonicalCommand: "{}",
    ...
    expect(receipt).toMatchObject({
      formatVersion: 1,
      result: { positionId: "position-1", journalEntryIds: ["journal-1"] },
    })
  })
