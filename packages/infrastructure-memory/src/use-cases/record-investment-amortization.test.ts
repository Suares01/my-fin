...
function useCase(f: Awaited<ReturnType<typeof setup>>) {
  return new RecordInvestmentAmortization(
...
describe("RecordInvestmentAmortization", () => {
  it("reduces cost, preserves units and records the normative receipt and gain", async () => {
    const f = await setup()
    expect(await useCase(f).execute(amortization(f))).toEqual({
      ok: true,
      value: {
        requestId: "amortization-1",
        positionId: "position-1",
  ...
  it("separates retained fees and taxes in the single amortization journal", async () => {
    ...
    ).toEqual(["OPENING_ALLOCATION", "AMORTIZATION"])
  })
  ...
  it("replays the same amortization without another operation or journal", async () => {
    ...
    expect(await useCase(f).execute(amortization(f))).toMatchObject({
      ok: true,
  ...
  it("rejects a position from another book without writing a receipt", async () => {
    ...
    expect(
      await useCase(f).execute({ ...amortization(f), bookId: other.value.id })
    ).toMatchObject({ ok: false, error: { code: "BOOK_MISMATCH" } })
    expect(
      f.h.store.getInvestmentRequest(other.value.id as never, "amortization-1")
    ).toBeUndefined()
  })
})
