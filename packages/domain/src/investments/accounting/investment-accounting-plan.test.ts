...
describe("planInvestmentAccounting", () => {
  it.each([
    ...
    [
      "external purchase fees",
      { feesMinor: "10", taxesMinor: "5" },
      "1000",
      "-1015",
      [
        ["bank", "-1015"],
    ...
    [
      ...
      { type: "APPLICATION", cashMode: "INTERNAL_CASH" },
      "1000",
      "-1000",
      [],
    ],
    ...
    expect(
      result.postings.map(({ accountId, amountMinor }) => [
        accountId,
        amountMinor,
      ])
    ).toEqual(expected)
  ...
  it.each([
    [
      "purchase without external account",
      { accounts: { ...accounts, externalAccountId: undefined } },
    ],
    [
      "sale with missing gain category",
      {
        type: "SALE",
        capitalMinor: "1",
        grossAmountMinor: "2",
        cashMode: "INTERNAL_CASH",
        accounts: { ...accounts, gainCategoryId: undefined },
      },
    ],
    [
      "loss with missing loss category",
      {
        type: "SALE",
        capitalMinor: "2",
        grossAmountMinor: "1",
        cashMode: "INTERNAL_CASH",
        accounts: { ...accounts, lossCategoryId: undefined },
      },
    ],
    [
      "income with negative cash",
      {
        type: "INCOME",
        capitalMinor: "0",
        grossAmountMinor: "1",
    ...
    expect(() => plan(input as Partial<InvestmentAccountingPlanInput>)).toThrow(
      "Invalid investment accounting plan"
    )
  })
})
