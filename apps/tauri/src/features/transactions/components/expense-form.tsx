...
import { expenseDraftSchema, transactionErrorMessage } from "../transaction-form-model.js"
...
export type ExpenseTransactionDraft = Omit<Extract<JournalBusinessDraft, { readonly type: "INCOME" | "EXPENSE" }>, "type"> & { readonly type: "EXPENSE" }
type ExpenseFormProps = { readonly initialDraft?: ExpenseTransactionDraft; readonly pending?: boolean; readonly submitError?: unknown; readonly refreshWarning?: boolean; readonly onSubmit: (draft: ExpenseTransactionDraft) => Promise<void>; readonly onCancel: () => void }
function displayAmount(value?: string) { if (!value) return ""; const amount = BigInt(value); return `${amount / 100n},${String(amount % 100n).padStart(2, "0")}` }

export function ExpenseForm({ initialDraft, pending = false, submitError, refreshWarning = false, onSubmit, onCancel }: ExpenseFormProps) {
  ...
  if (options.missingCategories) return <MissingResource resource="categoria" href="/categories" />
  const disabled = pending || conflictLocked
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (disabled || options.bookId === null) return
    const parsed = expenseDraftSchema.safeParse({ type: "EXPENSE", accountId, categoryId, amountMinor, currency: options.baseCurrency ?? "", occurredOn, description })
    ...
    try { setErrors({}); setLocalError(null); await onSubmit(parsed.data) } catch (error) { setLocalError(error); if (isConflict(error)) setConflictLocked(true) }
  }
  return <form className="flex flex-col gap-6" onSubmit={submit} aria-busy={disabled}>
    ...
    <SelectField id="expense-account" label="Conta" value={accountId} options={options.accounts} error={errors.accountId} disabled={disabled} onChange={setAccountId} />
    <SelectField id="expense-category" label="Categoria" value={categoryId} options={options.categories} error={errors.categoryId} disabled={disabled} onChange={setCategoryId} />
    ...
    <div className="flex gap-3 justify-end"><Button type="button" variant="ghost" disabled={disabled} onClick={onCancel}>Cancelar</Button><Button type="submit" disabled={disabled}>{pending && <Spinner aria-hidden="true" data-icon="inline-start" />}{pending ? "Salvando despesa" : "Salvar despesa"}</Button></div>
  </form>
}
function SelectField({ id, label, value, options, error, disabled, onChange }: { readonly id: string; readonly label: string; readonly value: string; readonly options: readonly { id: string; name: string }[]; readonly error?: string; readonly disabled: boolean; readonly onChange: (value: string) => void }) { return <Field data-invalid={error ? "true" : undefined}><FieldLabel htmlFor={id}>{label}</FieldLabel><select id={id} value={value} disabled={disabled} aria-invalid={Boolean(error)} onChange={(event) => onChange(event.currentTarget.value)}><option value="">Selecione</option>{options.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select><FieldError>{error}</FieldError></Field> }
