...
function renderList(
  ...
  return { props, ...render(<TransactionList {...props} />) }
}
describe("TransactionList", () => {
  afterEach(cleanup)
  it("uses list geometry while loading", () => {
    renderList({ isPending: true })
    expect(
      screen.getByLabelText("Carregando transações").getAttribute("aria-busy")
    ).toBe("true")
  })
  it("shows retry for initial failure", () => {
    const { props } = renderList({ isError: true })
    fireEvent.click(screen.getByRole("button", { name: "Tentar novamente" }))
    expect(props.onRetry).toHaveBeenCalledTimes(1)
  })
  it("shows create action for unfiltered empty state", () => {
    const { props } = renderList({ items: [] })
    fireEvent.click(screen.getByRole("button", { name: "Nova transação" }))
    expect(props.onCreate).toHaveBeenCalledTimes(1)
  })
  it("shows filter reset for local empty state", () => {
    const { props } = renderList({ items: [], status: "CANCELLED" })
  ...
  it("renders signed income", () => {
    renderList()
    expect(screen.getAllByText("+", { exact: false }).length).toBeGreaterThan(0)
  })
  it("renders signed expense", () => {
    renderList({ items: [chain({ type: "EXPENSE" })] })
    expect(screen.getAllByText("-", { exact: false }).length).toBeGreaterThan(0)
  })
  it("renders neutral transfer", () => {
    renderList({ items: [chain({ type: "TRANSFER" })] })
    expect(screen.queryByText("+R$ 100,00")).toBeNull()
  })
  it("renders desktop semantic columns", () => {
    renderList()
    expect(screen.getByRole("columnheader", { name: "Contexto" })).toBeTruthy()
    expect(screen.getByRole("columnheader", { name: "Ações" })).toBeTruthy()
  })
  it("keeps mobile essential content without horizontal scroll class", () => {
    renderList()
    expect(screen.getByLabelText("Lista de transações").className).toContain(
      "lg:hidden"
    )
  })
  it("expands a chain by keyboard-operable button", () => {
    renderList()
  ...
  it("uses stable chain identity for rows", () => {
    renderList({
      items: [chain(), chain({ chainId: "chain-2", description: "Mercado" })],
    })
    expect(screen.getAllByText("Mercado").length).toBeGreaterThan(0)
  })
  it("includes date and status", () => {
    renderList()
    expect(screen.getAllByText(/2026-09-03/).length).toBeGreaterThan(0)
