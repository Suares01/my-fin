...
function ChainValue({ item }: { readonly item: JournalChainListItem }) {
  return (
    <span>
      ...
      <FormattedMoney amountMinor={item.amountMinor} currency={item.currency} />
    </span>
  )
}
    ...
    return (
      <div className="rounded-xl border border-dashed p-6 text-center">
        ...
        <Button
          ...
          onClick={status === "ALL" ? onCreate : onResetFilters}
        >
          {status === "ALL" ? "Nova transação" : "Limpar filtros"}
        </Button>
      </div>
    )
  return (
